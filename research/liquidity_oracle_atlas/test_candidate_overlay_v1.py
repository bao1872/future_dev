...
Committed (clean-checkout) tests for the candidate-zone audit overlay
...
These committed tests MUST run in a clean checkout (``git clone && pytest``), so
they use only SYNTHETIC candidate rows (hand-built gate-shaped DataFrames) plus
the canonical FORMING-MTF environment built from the COMMITTED raw 5m frame
(no untracked parquet artifact required):

  * exact time alignment (decision_time <-> built track availability_time)
  ...
  * non-5m track returns empty
  * the canonical FORMING-MTF audit state is the forming bar, not last-closed

Full-history artifact integration (real artifact alignment, viewport rendering
against the real AG artifact) lives in ``verify_candidate_gate_r3_artifacts.py``
