...
def _log_factorial(y):
    ...
    return np.array([lgamma(int(v) + 1) for v in y], dtype=np.float64)
...
def _hurdle_nll(y, p0, rate):
    ...
    y    : (n,) int increment (>=0)
    p0   : (n,) P(increment > 0)
    rate : (n,) zero-truncated Poisson rate (positive)
    """
    y = np.asarray(y, dtype=np.float64)
    p0 = np.asarray(p0, dtype=np.float64)
    rate = np.maximum(np.asarray(rate, dtype=np.float64), 1e-6)
    poisson_nll = rate - y * np.log(rate) + _log_factorial(y)
    return np.where(y == 0, -np.log1p(-p0), -np.log(p0) + poisson_nll)
...
def fit_constant_count_head(Ytr, Yev):
    ...
    for j in range(Ytr.shape[1]):
        ...
        pos = yt[yt > 0]
        rate = float(np.mean(pos)) if len(pos) else 1.0
...
def fit_count_head(Xtr, Ytr, Xev, Yev):
    ...
    for j in range(Ytr.shape[1]):
        ...
        pos = yt > 0
        if pos.sum() > 0:
            pr = PoissonRegressor(alpha=1.0, max_iter=5000).fit(Xtr[pos], yt[pos])
            rate_tr = pr.predict(Xtr)
            rate_ev = pr.predict(Xev)
        else:
