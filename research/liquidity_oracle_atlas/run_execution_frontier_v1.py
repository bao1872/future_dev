...
def metric(g):
    ...
    return dict(n_signals=len(g),n_trades=len(executed),entry_invalid_n=int((g.status=="LAG1_ENTRY_INVALID").sum()),
...
def main():
    ...
    for sym,g in m.groupby("symbol",sort=False):
        ...
        entry_i=gi.contact_bar_index.astype(int).to_numpy()+2+gi.h.astype(int).to_numpy()
        ...
        for j,r in enumerate(gi.itertuples(index=False)):
            ...
            if not geometry[j]: rows.append(dict(**base,status="LAG1_ENTRY_INVALID",R_lower=np.nan,R_upper=np.nan,
                ambiguous=False,censored=True,entry_time=pd.NaT)); continue
    ...
    gate=bool((whole.EV_R_lower_per_signal>0).all()); verdict="LAG1_EXECUTION_EDGE_SURVIVES" if gate else "STOP_NO_ROBUST_LAG1_EXECUTION_EDGE"
