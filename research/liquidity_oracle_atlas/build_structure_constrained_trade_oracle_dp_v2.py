"""
build_structure_constrained_trade_oracle_dp_v2
===============================================

Intraday Structure-Constrained Trade Oracle DP V2 — ONE ENTRY PER PROXIMITY
EPISODE.

Task ID : FUTURE-INTRADAY-DP-ORACLE-R2-ONE-ENTRY-PROXIMITY
Base SHA: 3b5f57c57c1c2f770fa8eb8220af9bbc46a2d6df

V2 frozen change vs V1
-----------------------
The only new constraint is frequency control via a *proximity episode entry
right*. Everything else (5m clock, next-open fill, (trading_day, segment)
units, no overnight, gross-PnL objective, cost interface, 8-bit TF x {SR,LIQ}
provenance) is carried over unchanged.

  * V1 entry gate  = current 5m range TOUCHES a pre-existing SR/Liquidity zone
    (delta = 0).  A new position may open whenever eligible.
  * V2 entry gate  = current 5m range lies within ``ENTRY_PROX_ATR`` (0.50) ATR
    of ANY pre-existing SR/Liquidity zone (distance-based proximity; true touch,
    entering, or merely approaching all count the same).  Within one *continuous
    proximity episode* the oracle may open AT MOST ONE new position; it may WAIT
    for the best bar inside the episode.  Once any new-entry action occurs the
    entry right is consumed (``armed -> 0``).  Exit is always free.  Re-entry /
    reversal is forbidden until price leaves ALL structure proximity and later
    re-enters (which re-arms to ``armed = 1``).

State is now ``(position p in {-1,0,+1}, entry_right armed q in {0,1})`` -> 6
states.  Bellman:

    V_t(p, q) = max_a [ a (O_{t+2} - O_{t+1}) - c_t |a - p|
                       + V_{t+1}(a, q') ]

    q' = 1                         if P_{t+1} == 0
         0                         if P_{t+1} == 1 and NewEntry(p, a) == True
         q                         otherwise

    NewEntry(p, a) = (p == 0 and a != 0) or (p != 0 and a == -p)

Complexity: O(N * 6 * 3) = O(N).  Single backward scan, no future-exit scan, no
per-decision history recompute.  The proximity kernel reuses the canonical
geometry streaming (prev geometry carries ATR); the event/episode machinery
stays at zero (mask_only fast path).

Canonical owner reused (READ ONLY for indicators):
...
MATH_VERSION = "intraday_dp_oracle_r2_one_entry_proximity"
TASK_ID = "FUTURE-INTRADAY-DP-ORACLE-R2-ONE-ENTRY-PROXIMITY"

# unit terminal vocabulary (shared with V1)
DATA_END = "DATA_END"
...
# --------------------------------------------------------------------------- #
# Proximity episode id (exogenous P_t runs; resets at unit boundary)            #
# --------------------------------------------------------------------------- #
...
def _walk_unit_path(core: Dict[str, np.ndarray], proximity_any: np.ndarray, s: int, e: int):
    """Backtrack the optimal (p, q) path from the frozen (Flat, armed=1) start.

    ``proximity_any`` is the GLOBAL array; ``s`` / ``e`` are the unit bounds so we
    can rewire ``q`` from ``proximity_any[s + local + 1]`` (the next decision's
    proximity), exactly as the DP kernel did.
    """
...
def _dp_from_stream_v2(
    ...
    *,
    ...
    proximity_bits = np.asarray(res["proximity_bits"], dtype=np.int64)[:n]
    proximity_any = np.asarray(res["proximity_any"], dtype=bool)[:n]
...
def run_symbol_dp_v2(
    ...
    *,
    ...
    """Single-pass production runner for one symbol (V2)."""
    t0 = time.perf_counter()
    info = build_base_frame(symbol, counters)
    base = info["base"]
    res = _stream_from_base(
        info["base"], info["form"], info["seg_completed"], counters,
        max_bars, symbol, False, False, False, True, True,
    )
    t1 = time.perf_counter()

    if profile_memory and tracemalloc is not None:
        tracemalloc.start()
    out = _dp_from_stream_v2(symbol, base, res, counters, cost_points=cost_points)
    if profile_memory and tracemalloc is not None:
...
def run_base_dp_v2(
    ...
    *,
    ...
    res = stream_from_base(
        ...
        capture_proximity=True, emit_events=False, mask_only=True,
    )
    return _dp_from_stream_v2(symbol, base, res, counters, cost_points=cost_points)


def run_arrays_dp_v2(
    time_arr,
    trading_day_arr,
    o,
    h,
    ...
    *,
    ...
    """TRUE prefix runner for the N/2N/4N TP benchmark (V2)."""
    if profile_memory and tracemalloc is not None:
        tracemalloc.start()
    t0 = time.perf_counter()
    info = build_base_from_arrays(time_arr, trading_day_arr, o, h, l, c, disc, counters)
    base = info["base"]
    res = _stream_from_base(
        info["base"], info["form"], info["seg_completed"], counters,
        None, symbol, False, False, False, True, True,
    )
    t1 = time.perf_counter()
    out = _dp_from_stream_v2(symbol, base, res, counters, cost_points=cost_points)
    t2 = time.perf_counter()
...
def run_stage_dp_v2(
    ...
    *,
    ...
    for sym in symbols:
        results.append(
            run_symbol_dp_v2(
                sym, counters, max_bars, cost_points=cost_points,
                profile_memory=profile_memory,
            )
        )
    frames = [build_artifact_frames_v2(r) for r in results]
    oracle_actions = (
    ...
    return {
        ...
        "per_symbol": {
            r["symbol"]: {"decisions": int(len(r["sel"])), "trades": len(r["trades"]),
                          "units": len(r["units"])}
            for r in results
        },
        "_results": results,
...
# --------------------------------------------------------------------------- #
def build_artifact_frames_v2(result: Dict[str, Any]) -> Dict[str, pd.DataFrame]:
    sel = result["sel"]
    times = result["time"]
    dec = result["decision"]
    ...
    action_cols = {
        ...
        "proximity_bits": dec["proximity_bits"][sel],
        "proximity_any": dec["proximity_any"][sel],
        "proximity_episode_id": dec["proximity_episode_id"][sel],
...
# --------------------------------------------------------------------------- #
ARTIFACT_ROOT_DIRNAME = "intraday_dp_oracle_r2_one_entry_proximity"
...
ORACLE_VIEWER_TRADE_COLUMNS_V2 = (
    "direction",
    "entry_fill_time",
    "entry_fill_price",
    "exit_fill_time",
...
def write_oracle_artifact_v2(
    ...
    *,
    ...
    frames = build_artifact_frames_v2(result)
...
def load_oracle_artifact_v2(
    ...
    *,
    ...
    missing = [c for c in ORACLE_VIEWER_TRADE_COLUMNS_V2 if c not in trades.columns]
    if missing:
...
# --------------------------------------------------------------------------- #
def check_oracle_invariants_v2(result: Dict[str, Any]) -> Dict[str, int]:
    ...
    # max new entries per non-trivial proximity episode
